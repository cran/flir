...
test_that("rep_len_linter blocks simple disallowed usages", {
  ...
  #
  # TODO: not convinced this is worth supporting
...
test_that("fix works", {
  linter <- rep_len_linter()
  expect_snapshot(fix_text("rep(1:3, length.out = 5)", linters = linter))
  expect_snapshot(fix_text("rep(x, length.out = 5)", linters = linter))
})
